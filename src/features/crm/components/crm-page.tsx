...
import type { CRMEntry } from '@/shared/types';
...
const CRMPage = () => {
    ...
    const filteredLeads = (crmEntries || []).filter((lead: CRMEntry) => {
        const matchesSearch =
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-slate-50/50 border-b border-slate-100">
                            ...
                            <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Ubicacion / Sup.</th>
                            <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Inversion</th>
                        ...
                        {filteredLeads.length === 0 ? (
                            <tr>
                                <td colSpan={5} className="px-8 py-20 text-center text-slate-400 font-medium">
                                    No se encontraron leads. Guarda proyectos desde la seccion de Presupuesto.
                                </td>
                            </tr>
                        ) : (
                            filteredLeads.map((lead: CRMEntry) => (
                                <tr key={lead.id} className="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-[10px] font-bold text-slate-400 flex items-center gap-2 mt-1">
                                                    <span className="text-orange-600">{lead.budgetNumber || 'N/A'}</span> {lead.cuit && <>&bull; <span className="text-slate-500">{lead.cuit} &bull; </span></>}{formatDate(lead.date)}
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                                        ...
                                        <div className="flex gap-4 mt-2">
                                            <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{lead.area?.toFixed(1) || '0'} m2</div>
                                    ...
                                    <td className="px-8 py-6">
                                        ...
                                        >
                                            {['Nuevo', 'Presupuesto', 'Seguimiento', 'Revision', 'Ganado', 'Perdido'].map(s => (
