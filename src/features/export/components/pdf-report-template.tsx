...
import type { GeometryResult, BudgetItem } from '@/shared/types';

interface PDFReportTemplateProps {
  snapshots3D: string[] | null;
  floorPlanImage: string | null;
  geo: GeometryResult;
  quantities?: Record<string, number>;
}

const PDFReportTemplate: React.FC<PDFReportTemplateProps> = ({ snapshots3D, floorPlanImage, geo, quantities = {} }) => {
  ...
  return ReactDOM.createPortal(
    <div id="hidden-report-container" style={{
      ...
      <div className="pdf-page-fixed" style={{ height: '1587px', padding: '60px', position: 'relative', display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            <h1 style={{ margin: 0, fontSize: '48px', fontWeight: 900, color: '#ea580c' }}>COTIZACION</h1>
            <p style={{ margin: 0, fontSize: '18px', color: '#64748b', fontWeight: 700 }}>REPORTE TECNICO V1.0</p>
          </div>
        </div>
        ...
        <div style={{ marginTop: '100px', flex: 1 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px', marginTop: '60px' }}>
            ...
            <div>
              <p style={{ fontSize: '14px', color: '#94a3b8', margin: '0 0 5px 0', fontWeight: 800, textTransform: 'uppercase' }}>UBICACION</p>
            ...
            <div>
              ...
              <p style={{ fontSize: '24px', fontWeight: 700, margin: 0 }}>{geo.areaPiso?.toFixed(2)} m2</p>
            </div>
          </div>
        </div>

        {snapshots3D && snapshots3D[0] && (
          <div style={{ width: '100%', height: '800px', backgroundColor: '#f8fafc', borderRadius: '40px', overflow: 'hidden', border: '5px solid #ffffff', marginTop: '40px' }}>
        ...
        <div style={{ marginTop: 'auto', borderTop: '2px solid #f1f5f9', paddingTop: '30px', display: 'flex', justifyContent: 'space-between', color: '#94a3b8', fontSize: '14px', fontWeight: 700 }}>
          <span>LA FABRICA DEL PANEL | SISTEMA CONSTRUCTIVO SIP</span>
          <span>www.lafabricadelpanel.com.ar</span>
        </div>
      </div>

      {/* PAGE 2: PLANTA Y GEOMETRIA */}
      <div className="pdf-page-fixed" style={{ height: '1587px', padding: '60px', position: 'relative', display: 'flex', flexDirection: 'column', boxSizing: 'border-box', borderTop: '40px solid #f8fafc' }}>
        <h3 style={{ fontSize: '32px', fontWeight: 900, color: '#1e293b', marginBottom: '40px', borderLeft: '10px solid #ea580c', paddingLeft: '20px' }}>PLANTA TECNICA Y GEOMETRIA</h3>
        ...
        <div style={{ marginTop: 'auto', display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '40px', paddingBottom: '40px' }}>
          {[
            { label: 'AREA DE PLANTA', value: `${geo.areaPiso?.toFixed(2)} m2` },
            { label: 'PERIMETRO EXTERIOR', value: `${geo.perimExt?.toFixed(2)} ml` },
            { label: 'AREA MUROS PROYECTADA', value: `${geo.areaMuros?.toFixed(2)} m2` },
            { label: 'PERIMETRO DE ABERTURAS', value: `${geo.perimAberturas?.toFixed(2)} ml` },
            { label: 'CANTIDAD TOTAL PANELES', value: `${geo.totalPaneles} UNID` },
            { label: 'ALTURA MAXIMA RIDGE', value: `${dimensions.ridgeHeight} m` },
          ].map((item, idx) => (
            <div key={idx} style={{ padding: '40px', backgroundColor: '#f8fafc', borderRadius: '35px', textAlign: 'left', border: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <p style={{ margin: 0, fontSize: '14px', color: '#94a3b8', fontWeight: 900, textTransform: 'uppercase' }}>{item.label}</p>
      ...
      <div className="pdf-page-fixed" style={{ height: '1587px', padding: '60px', position: 'relative', display: 'flex', flexDirection: 'column', boxSizing: 'border-box', borderTop: '40px solid #f8fafc' }}>
        ...
        {(() => {
          ...
          const categories = [...new Set(budgetItems.map((i: BudgetItem) => i.category))];

          return categories.map(cat => {
            const catItems = budgetItems.filter((i: BudgetItem) => i.category === cat);
            ...
            return (
              <div key={cat} style={{ marginBottom: '30px' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
                  ...
                  <tbody>
                    {catItems.map((row: BudgetItem, i: number) => (
        ...
        <div style={{ marginTop: 'auto', padding: '40px', backgroundColor: '#ea580c', borderRadius: '30px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: '#ffffff' }}>
          <div>
            <p style={{ margin: 0, fontSize: '14px', fontWeight: 800, textTransform: 'uppercase', opacity: 0.8 }}>Inversion Total Estimada</p>
          ...
          <div style={{ textAlign: 'right', fontSize: '14px', fontWeight: 600 }}>
            <p style={{ margin: 0 }}>* Sujeto a cambios segun flete y montaje</p>
            <p style={{ margin: 0 }}>Valido por 7 dias desde la fecha.</p>
          </div>
        </div>
      </div>
      ...
      <div className="pdf-page-fixed" style={{ height: '1587px', padding: '60px', position: 'relative', display: 'flex', flexDirection: 'column', boxSizing: 'border-box', borderTop: '40px solid #f8fafc' }}>
        ...
        <div style={{ marginTop: '60px', padding: '40px', backgroundColor: '#f1f5f9', borderRadius: '30px' }}>
          <h4 style={{ margin: '0 0 15px 0', fontSize: '18px', fontWeight: 900 }}>NOTAS TECNICAS</h4>
          <p style={{ margin: 0, fontSize: '14px', lineHeight: 1.6, color: '#475569' }}>
            El sistema SIP (Structural Insulated Panels) ofrece un aislamiento termico superior, reduciendo el consumo energetico hasta un 60%.
            Este reporte ha sido generado de forma dinamica basandose en la modulacion estandar de paneles de 1.22 x 2.44m.
            Los paneles utilizan nucleo de EPS de densidad estandar para maxima eficiencia.
            Las aberturas indicadas se consideran cortes precisos realizados en fabrica.
          </p>
        </div>
      </div>
      {/* PAGE 5: CONDICIONES Y BENEFICIOS */}
      <div className="pdf-page-fixed" style={{ height: '1587px', padding: '60px', position: 'relative', display: 'flex', flexDirection: 'column', boxSizing: 'border-box', borderTop: '40px solid #f8fafc' }}>
        <h3 style={{ fontSize: '32px', fontWeight: 900, color: '#1e293b', marginBottom: '40px', borderLeft: '10px solid #ea580c', paddingLeft: '20px' }}>TERMINOS Y BENEFICIOS</h3>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px', marginTop: '20px' }}>
            ...
            <div style={{ backgroundColor: '#f8fafc', borderRadius: '30px', padding: '30px', border: '1px solid #f1f5f9' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                {project.projectInfo?.showEarlyPaymentDiscount !== false && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e2e8f0', paddingBottom: '15px' }}>
                    <div>
                      ...
                      <p style={{ margin: '4px 0 0 0', fontSize: '13px', color: '#64748b', fontWeight: 700 }}>CIERRE EN 7 DIAS</p>
                    </div>
                ...
                <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                  ...
                  <p style={{ margin: 0, fontSize: '13px', color: '#64748b', fontWeight: 700 }}>Acompanamiento integral y asesoria tecnica personalizada.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {[
                { title: 'Validez de la oferta', desc: 'La presente cotizacion tiene una vigencia de 7 dias corridos (sujeta a disponibilidad de stock).' },
                { title: 'Politica impositiva', desc: 'Los precios expresados corresponden a valores netos y no incluyen IVA.' },
                { title: 'Condicion de reserva', desc: 'Los descuentos por pronto pago se aplican validando la compra dentro del plazo de vigencia mencionado.' },
                { title: 'Logistica', desc: 'El presupuesto no incluye gastos de envio ni descarga en obra.' },
              ].map((item, i) => (
            ...
            <div style={{ marginTop: '40px', padding: '30px', backgroundColor: '#fff7ed', borderRadius: '25px', border: '1px solid #ffedd5' }}>
              ...
              <p style={{ margin: 0, fontSize: '14px', lineHeight: 1.6, color: '#7c2d12' }}>
                Los valores y descuentos estan sujetos a los plazos establecidos. El inicio de la produccion queda supeditado a la confirmacion de la sena y disponibilidad de materiales.
              </p>
            </div>
          </div>
        </div>
