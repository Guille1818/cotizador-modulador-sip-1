...
import FloorPlan from '@/features/engineering/components/floor-plan';
import FacadeView from '@/features/engineering/components/facade-view';
import {
  FileText,
  Loader2,
  Home,
...
import type { BudgetItem, GeometryResult } from '@/shared/types';

const FACTORY_NAME = "LA FABRICA DEL PANEL";
...
const ProjectFooter: React.FC<ProjectFooterProps> = ({ price, pageNumber, hidePrice }) => (
      ...
      <div className="flex items-center gap-6 text-right">
        {!hidePrice && (
          <div className="bg-slate-50 px-4 py-2 rounded-2xl border border-slate-100">
            <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Inversion Final</p>
...
const Export: React.FC = () => {
  ...
  const { geo, quantities } = useMemo(() => {
    try {
      ...
      return { geo: {} as GeometryResult, quantities: {} as Record<string, number> };
    }
  }, [dimensions, selections, interiorWalls, openings, facadeConfigs, project, foundationType, structureType, prices]);

  const { items: budgetItems, total: finalTotal, subtotal: _subtotalWithMarkup } = useMemo(() => {
    try {
      return calculateBudget(quantities, prices, project);
    } catch (e) {
      console.error("Budget calculation error:", e);
      return { items: [] as BudgetItem[], total: 0, subtotal: 0 };
    }
  }, [quantities, prices, project]);

  const { panelsItems, suppliesPages, panelsSubtotal, suppliesSubtotal } = useMemo(() => {
    const panels = budgetItems.filter((item: BudgetItem) => item.category === '1. SISTEMA DE PANELES');
    const supplies = budgetItems.filter((item: BudgetItem) => item.category !== '1. SISTEMA DE PANELES');
    ...
    return {
      panelsItems: panels,
      suppliesPages: pages,
      panelsSubtotal: panels.reduce((acc: number, i: BudgetItem) => acc + (i.total || 0), 0),
      suppliesSubtotal: supplies.reduce((acc: number, i: BudgetItem) => acc + (i.total || 0), 0),
    };
  }, [budgetItems]);
  ...
  const handleExportPDF = (): void => { window.print(); };
  ...
  const handleExportImage = async (): Promise<void> => {
    ...
    try {
      ...
      for (let i = 0; i < pages.length; i++) {
        const canvas = await html2canvas(pages[i] as HTMLElement, { scale: 1.5, useCORS: true, allowTaint: true, backgroundColor: '#ffffff' });
  ...
  if (!isReady) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-100 flex-col gap-4">
        ...
        <p className="text-slate-500 font-bold animate-pulse">Generando Reporte Tecnico...</p>
      </div>
    );
  }
          ...
          <div className="flex-1 flex flex-col border-[4px] border-slate-50 p-6 rounded-3xl relative z-10 overflow-hidden">
            ...
            <div className="bg-gradient-to-br from-slate-50 to-white rounded-2xl p-8 border-2 border-slate-100 mb-8 shadow-sm">
              ...
              <div className="grid grid-cols-4 gap-4">
                ...
                <div><div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Ubicacion</div><div className="text-lg font-black text-slate-900">{project.location || 'No especificada'}</div></div>
        ...
        <div className="pdf-page w-[210mm] h-[297mm] bg-white p-[10mm] flex flex-col text-slate-900 border border-slate-200 shadow-2xl hover:shadow-3xl hover:-translate-y-1 transition-all duration-300 mb-10 print:mb-0 print:border-none print:hover:shadow-none print:hover:translate-y-0">
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 flex-1">
              {[
                { t: 'Construccion Rapida', d: 'Reduccion del tiempo de obra hasta un 70%.', e: '\u26A1' },
                { t: 'Eficiencia Termica', d: 'Aislacion superior, ahorro de hasta 60% en energia.', e: '\uD83C\uDF21\uFE0F' },
          ...
          <div className="flex-1 flex flex-col gap-6">
            <div className="grid grid-cols-4 gap-4">
              {[
                ...
                { l: 'Superficie', v: `${geo?.areaPiso?.toFixed(1) || '0'}m2` },
                { l: 'Perimetro', v: `${geo?.perimExt?.toFixed(1) || '0'}m` },
              ].map((d, i) => (
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="bg-slate-900 rounded-2xl p-6 text-white space-y-4 shadow-xl">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center text-[11px]">
                    <span className="text-slate-400 font-bold uppercase tracking-wider">Nucleo Aislante:</span>
                    <span className="font-black text-white">70mm EPS Densidad Estandar</span>
                  </div>
                  <div className="flex justify-between items-center text-[11px]">
                    <span className="text-slate-400 font-bold uppercase tracking-wider">Eficiencia Termica:</span>
                  ...
                  <div className="flex justify-between items-center text-[11px]">
                    ...
                    <span className="font-black text-white">{((geo?.areaPiso ?? 0) * dimensions.height).toFixed(1)} m3 aprox.</span>
                  </div>
                  <div className="flex justify-between items-center text-[11px]">
              ...
              <div className="bg-orange-50 border border-orange-100 rounded-2xl p-6 space-y-4">
                ...
                <ul className="space-y-2">
                  {[
                    'Confort termico constante todo el ano.',
                    'Reduccion drastica en facturas de energia.',
                    'Ambiente libre de humedad y moho.',
                    'Aislamiento acustico superior para el descanso.',
          ...
          <div className="flex-1 flex flex-col justify-center space-y-12">
            <section className="space-y-6">
              <h3 className="text-[12px] font-black text-slate-900 uppercase tracking-[0.4em] flex items-center gap-4 text-center justify-center">
                ...
                Computo de Componentes SIP Principales
              </h3>
              <div className="grid grid-cols-5 gap-4 items-stretch">
                {[
                  { label: 'Muro Ext.', val: geo?.cantMurosExt || 0, show: selections.includeExterior !== false, highlight: false },
                  { label: 'Muro Int.', val: geo?.cantMurosInt || 0, show: selections.includeInterior !== false, highlight: false },
                  { label: 'Piso SIP', val: geo?.cantPiso || 0, show: foundationType !== 'platea' && selections.includeFloor !== false, highlight: false },
            ...
            <section className="space-y-6">
              <h3 className="text-[12px] font-black text-slate-900 uppercase tracking-[0.4em] flex items-center gap-4 text-center justify-center">
                ...
                Especificaciones Metricas de Obra
              </h3>
              <div className="grid grid-cols-2 gap-4">
                {[
                  { label: 'Area de Planta', val: geo?.areaPiso?.toFixed(2), unit: 'm2', icon: <Home size={28} /> },
                  { label: 'Perimetro Exterior', val: geo?.perimExt?.toFixed(2), unit: 'ml', icon: <Maximize size={28} /> },
                  { label: 'Area Muros Bruta', val: geo?.areaMurosBruta?.toFixed(2), unit: 'm2', icon: <Layout size={28} /> },
                  { label: 'Area Techos', val: geo?.areaTecho?.toFixed(2), unit: 'm2', icon: <Layout size={28} /> },
            ...
            <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden flex-1 overflow-y-auto">
              <table className="w-full text-left border-collapse border-b border-slate-100">
                <thead className="bg-slate-900 text-[8px] font-black text-white uppercase tracking-[0.2em] sticky top-0">
                  <tr>
                    <th className="px-5 py-3">Descripcion de Paneles</th>
                ...
                <tbody className="divide-y divide-slate-100 text-[9px]">
                  {panelsItems.map((item: BudgetItem) => (
            ...
            <div className="flex-1 flex flex-col gap-4 min-h-0">
              <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden flex-1 overflow-y-auto">
                <table className="w-full text-left border-collapse border-b border-slate-100">
                  <thead className="bg-slate-900 text-[8px] font-black text-white uppercase tracking-[0.2em] sticky top-0">
                    <tr>
                      <th className="px-5 py-3">Descripcion Insumos y Servicios</th>
                      <th className="px-2 py-3 text-center">Unidad</th>
                      <th className="px-2 py-3 text-center">Cantidad</th>
                      <th className="px-2 py-3 text-right">Unitario</th>
                      <th className="px-5 py-3 text-right">Total</th>
                  ...
                  <tbody className="divide-y divide-slate-100 text-[9px]">
                    {(() => {
                      ...
                      return pageItems.map((item: BudgetItem, idx: number) => {
                            ...
                            {showCategory && (
                              <tr className="bg-slate-50">
                                <td colSpan={5} className="px-5 py-2 text-[7px] font-black text-orange-600 uppercase tracking-widest">
                                  {item.category} {pageIdx > 0 && idx === 0 ? '(Continuacion)' : ''}
                                </td>
                              </tr>
                            )}
          ...
          <div className="flex-1 space-y-8">
            <div className="bg-slate-50 rounded-2xl p-8 border-2 border-slate-100">
              ...
              <div className="space-y-4">
                {[
                  { t: 'Pronto Pago (Cierre en 7 dias)', v: '6% OFF', c: 'text-emerald-600', s: finalTotal * 0.06, show: project.projectInfo?.showEarlyPaymentDiscount !== false },
            ...
            <div className="bg-slate-900 rounded-3xl p-[40px] border-4 border-white flex-col relative shadow-2xl overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-4 relative z-10">
                {['Vigencia: 7 dias corridos.', 'Precios netos (No incluyen IVA).', 'Descuentos sujetos a validacion de compra.', 'No incluye flete ni descarga.'].map((c, i) => (
