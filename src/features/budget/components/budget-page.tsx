...
import type { Selections, Product, BudgetItem, GeometryResult, Project } from '@/shared/types';
...
const BudgetPage = () => {
    ...
    React.useEffect(() => {
        ...
        Object.keys(defaultValues).forEach(key => {
            if ((selections as unknown as Record<string, unknown>)[key] === undefined) {
                missing[key] = defaultValues[key];
                needsSync = true;
            }
        });
        if (needsSync) {
            setSelections(missing as Partial<Selections>);
        }
    }, [selections, setSelections]);

    const { geo = {} as GeometryResult, quantities = {} as Record<string, number> } = useMemo(() =>
        fullCalculation(dimensions, selections, interiorWalls, openings, facadeConfigs, { ...project, perimeterWalls, interiorWalls } as Partial<Project>, foundationType, structureType, prices),
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        setProjectData({ [name]: value } as Record<string, string>);
    };
        ...
        <div className="max-w-[1200px] mx-auto px-4 md:px-0 space-y-6 md:space-y-8 animate-in fade-in duration-700 pb-20">
                ...
                <div className="lg:col-span-2 bg-white p-6 md:p-8 rounded-[40px] shadow-sm border border-slate-100 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Numero de Registro</label>
                        ...
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Ubicacion</label>
                            <div className="relative">
                                <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={16} />
                                <input
                                    type="text" name="location" value={project.location} onChange={handleInputChange}
                                    placeholder="Ej. Neuquen, Argentina"
                                    className="w-full bg-slate-50 border-none rounded-2xl py-3 pl-12 pr-4 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-orange-500 transition-all placeholder:text-slate-300"
                                />
                            </div>
                        </div>
                    ...
                    <div className="space-y-6 relative z-10">
                        <div className="flex justify-between items-start">
                            ...
                            <div className="text-right">
                                ...
                                <h3 className="text-3xl font-black">{geo?.areaPiso?.toFixed(1) || '0.0'} <span className="text-sm font-light text-slate-400">m2</span></h3>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            ...
                            <textarea
                                ...
                                placeholder={defaults.benefits || '- Rapidez, - Termico...'}
                            />
                        </div>
                        <div className="space-y-1.5">
            ...
            <div className="bg-white p-6 md:p-8 rounded-[40px] shadow-sm border border-slate-100">
                <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                        <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest text-left">Configuracion de Paneles</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
                    {[
                        { idField: 'exteriorWallId' as const, includeField: 'includeExterior' as const, label: 'Muros Exteriores', suffix: '@@EXTERIOR', filter: (p: Product) => p.id.includes('-E') && !p.id.includes('-INT') && !p.id.includes('PISO') },
                        { idField: 'interiorWallId' as const, includeField: 'includeInterior' as const, label: 'Tabiqueria Interior', suffix: '@@INTERIOR', filter: (p: Product) => p.id.includes('-INT') || p.id.includes('DECO') || p.id === 'OSB-70' },
                        {
                            idField: 'roofId' as const,
                            includeField: 'includeRoof' as const,
                            label: 'Cubierta / Techo',
                            suffix: '@@TECHO',
                            isRoof: true,
                            filter: (p: Product) => p.category === '1. SISTEMA DE PANELES' && (selections.roofSystem === 'sandwich' ? p.id.includes('SAND-') : (p.id.includes('TECHO') || p.id === 'COL-70' || p.id === 'CE-70' || p.id === 'SID-70' || p.id === 'OSB-FEN-70'))
                        },
                        {
                            idField: 'floorId' as const,
                            includeField: 'includeFloor' as const,
                            label: 'Entrepiso / Piso',
                            suffix: '@@PISO',
                            disabled: foundationType === 'platea',
                            filter: (p: Product) => p.id.includes('PISO')
                        },
                    ].map(cat => (
                        <div key={cat.idField} className="space-y-3 bg-slate-50/50 p-4 rounded-[28px] border border-slate-100">
                            ...
                            <select
                                value={(selections[cat.idField] as string) || ''}
                                onChange={(e) => {
                                    const newVal = e.target.value;
                                    setSelectionId(cat.idField, newVal as Selections[keyof Selections]);
                                    if (newVal) {
            ...
            <div className="bg-white p-6 md:p-8 rounded-[40px] shadow-sm border border-slate-100">
                <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest text-left">Computo metrico de unidades</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className={`bg-white p-6 rounded-[32px] border shadow-sm flex flex-col justify-between group transition-all ${project.projectInfo?.showEarlyPaymentDiscount === false ? 'opacity-50 border-slate-100 grayscale' : 'border-emerald-100 hover:border-emerald-200'}`}>
                        <div>
                            ...
                            <p className="text-[10px] text-slate-400 font-medium mt-1 leading-relaxed">Cierre temprano del presupuesto (valido por 7 dias).</p>
                        </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left notranslate" translate="no">
                        <thead className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            <tr>
                                <th className="px-8 py-5">Descripcion Tecnica</th>
                                ...
                                <th className="px-4 py-5 text-right">Accion</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                            {['1. SISTEMA DE PANELES', '2. MADERAS ESTRUCTURALES (PINO TRATADO)', '3. FIJACIONES Y ANCLAJES', '4. AISLACION Y SELLADO QUIMICO', '5. SERVICIOS Y EXTRAS'].map(category => {
                                const categoryItems = budgetItems.filter((item: BudgetItem) => item.category === category);
                                                ...
                                                <td className="px-4 py-5 text-right">
                                                    {item.isOverridden ? (
                                                        <button
                                                            ...
                                                            title="Restaurar valor automatico"
                                                        ...
                                                        <button onClick={() => setOverride(item.id, { qty: 0 })} title="Ocultar item" className="p-2 text-slate-300 hover:text-rose-500 transition-colors">
                                                            <X size={18} />
                                                        </button>
                                                    )}
                                                </td>
