...
const AdminPage = () => {
    const { prices, defaults, updateProduct, addProduct, deleteProduct, updateDefaults } = useStore();
    ...
    const handleAddItem = () => {
        ...
        addProduct({ ...newItem, id: crypto.randomUUID() } as Product);
    ...
    const formulaMapping: Record<string, string> = {
        ...
        'HBS_140': 'Encuentros SIP-SIP + Solera Piso/Madera',
        'HBS_160': 'Piso SIP y Techo SIP a vigas',
        'HBS_200': 'Vigas entrepiso a muro * 2',
        'TEL_HEX_4': 'Solera sobre estructura metálica 1 x 0.8m',
        'HERRAJE_ANGULO_90': 'Intersección viga principal × secundaria × 2',
        'TACO_N10': 'Encuentros SIP-mampostería',
        ...
        <div className="max-w-7xl mx-auto space-y-8 pb-20 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100 space-y-6 relative group">
                    ...
                    <textarea
                        ...
                        placeholder="Ej: Validez por 15 dias..."
                        onChange={(e) => {
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-slate-50/50 text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100">
                            <tr>
                                ...
                                <th className="px-4 py-5">Formula / Logica de Calculo</th>
                                <th className="px-8 py-5 text-right">Precio ($)</th>
                                <th className="px-8 py-5 text-right">Accion</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50 text-[11px]">
                            ...
                            <tr className="bg-orange-50/20">
                                ...
                                <td className="px-4 py-4">
                                    <div className="text-[9px] text-slate-400 font-bold italic">Manual / Estatico</div>
                                </td>
                            ...
                            {prices.map((item: Product) => {
                                ...
                                const formula = formulaKey ? formulaMapping[formulaKey] : 'Basado en Geometria';
            ...
            <div className="bg-slate-900 rounded-2xl p-10 shadow-3xl border border-slate-800 text-slate-300 relative overflow-hidden">
                ...
                <div className="flex items-center gap-6 mb-12 relative z-10">
                    ...
                    <div>
                        <h3 className="text-3xl font-black text-white uppercase tracking-tighter leading-tight">Motor de Calculo SIP</h3>
                        <p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-500/60 flex items-center gap-2">
                            <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span> Sistema de logica dinamica v2.6
                        </p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 relative z-10">
                    {[
                        { title: "SISTEMA DE PANELES", formula: "Area / 2.97", cond: "SIP Estandar", desc: "Muros Ext, Int, Piso y Techo. Incluye factor de aprovechamiento del panel estandar (1.22x2.44m)." },
                        { title: "TECHO SANDWICH", formula: "Area Bruta Techo (m2)", cond: "Si Sandwich = ON", desc: "Calculo directo por metro cuadrado de cubierta. No requiere despiece por paneles." },
                        { title: "VIGAS 3x6 (TECHO)", formula: "(Largo / 0.6) * Ancho * 1.1", cond: "Si Techo SIP = ON", desc: "Calculo de vigas estructurales cada 60cm con 10% extra por desperdicio y fijaciones." },
                        { title: "VIGAS 3x6 (PISO)", formula: "Area Piso * 2.5", cond: "Madera/Metal + PISO SIP", desc: "ML de madera para bastidor de piso. No se aplica si la fundacion es Platea de Hormigon." },
                        { title: "MADERA VINC. 2x3\"", formula: "(Paneles Muro/Tab/Piso) * 6.6", cond: "Siempre", desc: "Liston de pino macizo para union entre todos los paneles del sistema." },
                        { title: "SOLERA BASE 1x4\"", formula: "(Paneles Muro/Tab) * 1.3", cond: "Siempre", desc: "Pie de solera para apoyo y nivelacion de paneles verticales (Muros y Tabiques)." },
                        { title: "CLAVADERA 2x2\"", formula: "Perimetro Exterior * 5.5", cond: "Muros Exteriores", desc: "Listones para fijacion de revestimientos externos y terminaciones." },
                        { title: "ANCLAJES PLATEA", formula: "(P. Muro + Tab * 1.3) / 10", cond: "Solo si NO hay PISO", desc: "Calculo de Anclaje Quimico y Varillas (factor /5) para fijacion directa a la platea." },
                        { title: "TORNILLERIA FIX", formula: "Paneles * Factor (55 o 5.3)", cond: "Siempre", desc: "Calculo por unidad de tornillo fix para vinculantes y pie de solera." },
                        { title: "TORNILLERIA TORX", formula: "Paneles * 4 | (Techo+Piso)*3.5", cond: "Estructural", desc: "Tornillos Torx de alto rendimiento (120mm y 140mm) para encuentros estructurales." },
                        { title: "FIJACIONES CHAPA", formula: "Area Techo * 4.5", cond: "Cubierta", desc: "Tornillos Hexagonales con O-ring para fijacion de chapa o panel sandwich." },
                        { title: "SELLADO QUIMICO", formula: "(Paneles Totales * 8) / 25", cond: "Siempre", desc: "Espuma de Poliuretano 750cc para sellar todas las uniones del sistema SIP." },
                        { title: "PEGAMENTO PU", formula: "(Paneles Muro/Tab * 1.2) / 4", cond: "Soleras", desc: "Pegamento poliuretanico para vinculacion de soleras con la platea." },
                        { title: "SOLUCION HIDRICA", formula: "((P. Muro*1.3 + P. Tab*1.3)*0.15 + 30) / 20", cond: "Membrana Liquida", desc: "Logica de calculo para proteccion hidrica basada en cantidad de paneles y factor de rendimiento." }
                    ].map((item, i) => (
                ...
                <div className="mt-12 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="flex items-center gap-3">
                        ...
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Todos los calculos incluyen un factor de seguridad del +10% (x1.1)</p>
                    </div>
                </div>
            </div>
        </div>
